  ...
  <div>
    ...
    >
      ...
      <div v-show="getDetailUserSuccessed">
        <!-- <b-card-text>
          <h5 class="text-primary mt-2">Detail API Log</h5>
        </b-card-text> -->
        <b-form-group label="URL">
          <label class="form-control" readonly>{{ detailUser.endpoint }}</label>
        </b-form-group>
        <b-form-group label="Tanggal">
          <label class="form-control" readonly>{{ detailUser.created_at }}</label>
        </b-form-group>
        <b-form-group label="Sumber">
          <label class="form-control" readonly>{{ detailUser.source_ip_address }}</label>
        </b-form-group>
        <b-form-group label="Tujuan">
          <label class="form-control" readonly>{{ detailUser.destination_ip_address }}</label>
        </b-form-group>
        <b-form-group label="Request" label-size="auto">
        <textarea class="mb-2 mb-lg-0 form-control" v-model="detailUser.request" readonly></textarea>
        </b-form-group>
        <b-form-group label="Respons" label-size="auto">
        <textarea class="mb-2 mb-lg-0 form-control" v-model="detailUser.response" readonly></textarea>
        </b-form-group>
      </div>
    </b-modal>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'source_ip_address', label: 'Sumber' },
        { key: 'destination_ip_address', label: 'Tujuan' },
        { key: 'id', label: 'Aksi' },
      ],
      items: [],
      itemss: [
        {
          no: 1,
          endpoint: 'https://apilog/api',
          created_at: '29 oktober 2021',
          source_ip_address: 'Test Sumber',
          destination_ip_address: 'Test Tujuan'
        },
      ],
      title: {},
      deleteReason: '',
  ...
  methods: {
    ...
    async detail(idUser) {
      ...
      await axios({
        url: URLServices.externalAPILog.show,
        method: 'get',
        params: {
          request_id: uuid.v4(),
          id: idUser,
    ...
    async getDataTable() {
      ...
      axios({
        url: URLServices.externalAPILog.index,
        params: param,
        method: 'get',
        headers: {
          Authorization: `${TokenType} ${getToken()}`,
