            ...
            <template #cell(id)="data">
              <b-button
                ...
                :to="{
                  ...
                  params: {
                    selectedIdFormula: data.item.id,
                  },
                }"
              >
    ...
    >
      <form ref="form" @submit.stop.prevent="handleSubmit">
        ...
        <b-alert variant="warning" show class="p-1">
          ...
          Data yang telah dihapus tidak dapat dikembalikan. Klik tombol Hapus untuk melanjutkan
        </b-alert>
      </form>
    </b-modal>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'regulation_official_name', label: 'Nama Resmi Peraturan' },
        { key: 'publish_date', label: 'Tgl. Terbit' },
        // { key: 'end_date', label: 'Tgl. Akhir' },
  ...
  methods: {
    ...
    async showDelete(idUser, NamaUser) {
      ...
      this.title = {
        delete: `Apakah anda ingin menghapus data: ${NamaUser}?`,
      }
    ...
    async detail(idUser) {
      ...
      await axios({
        url: URLServices.BHPFrekuensi.destroy,
        method: 'get',
        params: {
          request_id: uuid.v4(),
          id: idUser,
    ...
    selectingID(id) {
      ...
      this.title = {
        delete: `Menghapus aturan data: ${this.selectedUser.user_name}`,
      }
    },
    ...
    async getDataTable() {
      ...
      await axios({
        url: URLServices.BHPFrekuensi.index,
        params: param,
        method: 'get',
        headers: {
          Authorization: `${TokenType} ${getToken()}`,
        ...
        .then(response => {
          if (response.data.response_code === '00') {
            CustomNotification.notif(
              ...
              'Tabel akan dimuat ulang'
            )
            this.$bvModal.hide('modalDelete')
            this.getDataTable()
          } else {
            CustomNotification.notif('warning', 'Gagal. Mohon coba kembali', response.data.response_message)
          }
        })
        .catch(error => {
          if (error.response.status && error.response.status === 401) {
    ...
    async download(IdRegulation) {
      const today = DateConvertion.getTimeNow()
      await axios({
        url: URLServices.BHPFrekuensi.download,
        method: 'post',
        data: {
          request_id: uuid.v4(),
