  ...
  <div>
    <b-row class="mb-4">
      <b-col class="text-right">
        ...
        <b-button
          variant="light"
          @click="seen = !seen" v-if="showTable"
          class="mr-1 text-primary"
        >
          ...
          >
            ...
            <template #cell(id)="data">
              ...
              <b-button
                ...
                variant="light"
                :to="{ name: 'employee-edit', params: {selectedUserID: data.item.id }}"
                btn-sm
                class="btn-sm text-primary"
              >
              ...
              <b-button
                ...
                @click="showDelete(data.item.id,data.item.full_name)"
              >
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'positions', label: 'Posisi', tdClass: 'Td-Role' },
  ...
  methods: {
    ...
    async detail(idUser) {
      ...
      await axios({
        ...
        .catch(error => {
          CustomNotification.notif('danger', 'Detail Employee Gagal Diperoleh', 'Mohon coba beberapa saat kembali')
    ...
    getDataTable() {
      ...
      axios({
        ...
        .then(response => {
          ...
          this.items = data.employee_list
          this.totalRows = recordsTotal
          this.isBusy = false
          this.showTable = this.totalRows > 0 || this.filter
        })
        .catch(error => {
          CustomNotification.notif('danger', 'Gagal memperbaharui data', 'Mohon coba beberapa saat kembali')
          if (error.response.status && error.response.status === 401) {
            localStorage.removeItem('accessToken')
            localStorage.removeItem('userData')
    ...
    deleteUser(bvModalEvt) {
      ...
      axios({
        url: URLServices.employee.destroy,
        data: {
          request_id: uuid.v4(),
        ...
        .then(response => {
          if (response.data.response_code === '00') {
            CustomNotification.notif('success', 'Berhasil menghapus data', 'Tabel akan direload')
            this.$bvModal.hide('modalDelete')
            this.getDataTable()
          } else {
            CustomNotification.notif('warning', 'axa', response.data.response_message)
          }
        })
        .catch(error => {
          CustomNotification.notif('warning', 'Gagal Menghapus', 'Mohon coba beberapa saat kembali')
          if (error.response.status && error.response.status === 401) {
            localStorage.removeItem('accessToken')
            localStorage.removeItem('userData')
            this.$router.go({ name: 'login' })
