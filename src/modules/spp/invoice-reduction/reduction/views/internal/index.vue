...
export default {
  ...
  data() {
    return {
      searchTitle: '',
      isBusy: true, // ubah true jika dengan ws
      seen: false,
      perPage: 10,
      ...
      deleteReason: '',
      nameState: null,
      showTable: false, // ubah true jika dengan ws
    }
  },
  computed: {
    sortOptions() {
  ...
  methods: {
    ...
    async detail(reducId) {
      this.$router.push({ name: 'reduction-detail' })
      localStorage.setItem('reductionId', reducId)
      console.log('INI APP ID :', reducId)
    ...
    async getDataTable() {
      ...
      await axios({
        url: URLServices.reduction.index,
        method: 'get',
        params: param,
        headers: {
