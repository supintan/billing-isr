...
<div>
  ...
  <b-card header-tag="header" v-show="submitVerification" class="mt-4">
    <template #header>
      <div>
         <h5 class="text-primary my-2">VERIFIKASI SURAT PERMOHONAN PENGURANGAN BHP</h5>
      </div>
    </template>
    <b-form>
      <b-row>
        <b-col sm="8" lg="8" md="8">
           <b-form-group label-cols="4" label="Keputusan Verifikasi*" >
            <b-form-radio-group
      ...
      <b-col offset-md="4">
          ...
          <b-button
            ...
            variant="success"
            class="ml-1"
             @click.prevent="saveVerification()"
             :disabled="!statusSave"
          >
            Simpan
          </b-button>
...
export default {
  ...
  computed: {
    statusSave() {
      return (this.selectedFormat && this.verification_notes)
    },
  },
  methods: {
    convDate(date) {
    ...
    createBase64File(e) {
      ...
      this.createRequestVariable(filez)
    },
    createRequestVariable(filez) {
      const data = {}
      ...
      data.supporting_documents_file_base64 = this.fileZip.join()
    ...
    async saveVerification() {
      ...
      if (this.selectedFormat === 'Setuju') {
          await axios({
            url: URLServices.reduction_appeal_submission.approve,
            method: 'post',
            data: {
              request_id: 'guid_v4',
              request_date_time: moment(new Date()).format('YYYY-MM-DD H:mm:s'),
              data: {
                id: this.ReductionAppealId,
                is_approved: true,
                template_id: 8,
                reviewer_notes: this.verification_notes,
              },
            },
            headers: {
            ...
            .then(result => {
              if (result.data.response_code === '00') {
                ...
                this.$router.push({ name: 'invoice-reduction-appeal-submission' })
              } else {
      ...
      if (this.selectedFormat === 'Tolak') {
          await axios({
            url: URLServices.reduction_appeal_submission.reject,
            method: 'post',
            data: {
              request_id: 'guid_v4',
              request_date_time: moment(new Date()).format('YYYY-MM-DD H:mm:s'),
            ...
            .catch(error => {
              CustomNotification.notif('danger', 'Gagal memperbaharui data', 'Mohon coba kembali beberapa saat kembali')
              this.showCreteUserForm = false
              if (error.response.status && error.response.status === 401) {
                localStorage.removeItem('accessToken')
                localStorage.removeItem('userData')
    ...
    async getDetailReductionAppeal(idRedAppeal, idRed) {
      await axios({
        ...
        .then(result => {
          ...
          if (((this.detailReductionAppeal.verification_decision === '-') && (this.ReductionSubmissionId === '-'))) {
            this.submitVerification = true
          }
        })
        .catch(error => {
          const errorMessage = `Mohon ulangi beberapa saat kembali. [${error.response.status}]`
