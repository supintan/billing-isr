  ...
  <div>
    <b-row class="mb-4">
      <b-col class="text-right">
        <b-button
          variant="light"
          :to="{ name: 'add-invoice-installment' }"
        ...
        >
          ...
          >
            ...
            <template #cell(review_date_time)="data">
              {{ convDate(data.review_date_time) }}
            </template>
            <template #cell(action)="data">
              <b-button
                ...
                :to="{ name: 'detail-invoice-installment', params: {idinstallment: data.item.id} }"
              >
                <feather-icon
                  icon="ZoomInIcon"
                  size="16"
...
export default {
  ...
  methods: {
    ...
    getDataTable() {
      ...
      axios({
        url: URLServices.installment.index,
        params: param,
        method: 'get',
        headers: {
          Authorization: `${TokenType} ${getToken()}`,
        ...
        .catch(() => {
          ...
          const errorMessage = `Gagal mendapatkan role. Mohon ulangi beberapa saat kembali.`
          CustomNotification.notif('warning', 'Warning', errorMessage)
        })
    },
