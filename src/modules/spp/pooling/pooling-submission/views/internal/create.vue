<template>
  <b-overlay :show="showCreteSimulasiForm" rounded="sm">
    <div>
      ...
      <div v-if="this.isStatus === 'DISETUJUI' || this.isStatus === 'MENUNGGU VERIFIKASI'">
        <b-card>
          <b-row>
            <b-col>
              <b-card-text>
            ...
            <b-col sm="12" lg="8" md="8">
              ...
              >
                <b-button
                    ...
                    type="button"
                    class="ml-1"
                    @click="downloadreview(idsubmissionpooling)"
                  >
                  <!-- {{id_pooling}} -->
                    Download File
                  </b-button>
      ...
      <div v-if="this.isStatus === 'MENUNGGU REVIEW'">
        <b-card>
          <b-row>
            <b-col>
              <b-card-text>
...
export default {
  ...
  data() {
    return {
      ...
      idappealpoolingsubmission: this.$route.params.idappealpoolingsubmission,
      ...
      fieldsPoolingChanged: [
        ...
        { key: 'pooling_billing_end_date', label: 'Tanggal Jatug Tempo Baru' },
  ...
  created() {
    this.getDetailPoolingSubmission()
  ...
  methods: {
    async getDetailPoolingSubmission() {
      await axios({
        ...
        .catch(error => {
          const errorMessage = `Gagal mendapatkan detail data. Mohon ulangi beberapa saat kembali. [${error.response.status}]`
    ...
    createRequestVariableFile(file) {
      ...
      data.review_letter_number = this.reviewer_letter_number
      data.template_id = this.templateId
      data.file_base64 = this.fileLoc
      data.id = this.id_pooling
      data.pooling_appeal_submission_id = this.idappealpoolingsubmission
      data.pooling_submission_id = this.idsubmissionpooling
      return data
    },
        ...
        await axios({
          ...
          .then(result => {
            if (result.data.response_code === '00') {
              const responseMessage = `Berhasil memperbaharui status review. [${result.data.response_message}]`
              CustomNotification.notif('success', 'Berhasil', responseMessage)
              this.$router.push({ name: 'review-poling' })
            } else {
