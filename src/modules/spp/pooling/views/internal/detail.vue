<template>
  <b-overlay :show="showCreteSimulasiForm" rounded="sm">
    ...
    <div v-if="verification_is_approved == true">
      <b-card>
        <b-row>
          <b-col>
            <b-card-text>
...
export default {
  ...
  data() {
    return {
      ...
      idsubmissionpooling: this.$route.params.idpoolingsubmission,
  ...
  methods: {
    ...
    async getDetailPooling() {
      await axios({
        ...
        .then(response => {
          ...
          this.verification_notes = this.dataAppeal.verification_notes
          ...
          this.review_letter_number = this.dataAppeal.review_letter_number

          if (this.pooling_submission_id != null) {
            this.getDetailPoolingSubmission(this.pooling_submission_id)
          }
        })
        .catch(error => {
          const errorMessage = `Gagal mendapatkan detail permohonan. Mohon ulangi beberapa saat kembali. [${error.response.status}]`
    ...
    async getDetailPoolingSubmission(value) {
      await axios({
        url: URLServices.hkppoolingsubmission.get,
        method: 'get',
        params: {
          request_id: this.guid_v4,
          id: value,
        ...
        .then(response => {
          this.dataPoolingSubmission = response.data.data
          // this.review_is_approved = this.dataPoolingSubmission.review_is_approved
          // this.reviewer_employee_full_name = this.dataPoolingSubmission.reviewer_employee_full_name
          // this.review_date_time = this.dataPoolingSubmission.review_date_time
          // this.reviewer_notes = this.dataPoolingSubmission.reviewer_notes
          // console.log(this.dataPoolingSubmission, 'pooling submission')
        })
        .catch(error => {
          const errorMessage = `Gagal mendapatkan detail review permohonan. Mohon ulangi beberapa saat kembali. [${error.response.status}]`
          CustomNotification.notif('error', errorMessage)
          if (error.response.status && error.response.status === 401) {
            localStorage.removeItem('accessToken')
            localStorage.removeItem('userData')
    ...
    createRequestVariableFile(file) {
      ...
      data.id = this.idsubmissionpooling
    ...
    createRequestUrl(selected) {
      ...
      if (selected === '1') {
        url = URLServices.hkppoolingsubmission.approve
      } else if (selected === '0') {
        url = URLServices.hkppoolingsubmission.reject
      }
      return url
    },
    async save() {
        ...
        await axios({
          ...
          .then(result => {
            if (result.data.response_code === '00') {
              const succesMessage = `Berhasil menambahkan user baru. [${result.data.response_message}]`
