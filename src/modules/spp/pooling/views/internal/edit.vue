        ...
        await axios({
          ...
          .then(result => {
            if (result.data.response_code === '00') {
              const successMessage = `Berhasil menambahkan user baru. [${result.response.response_message}]`
              CustomNotification.notif('success', 'Berhasil', successMessage)
              this.$router.push({ name: 'UserManagement' })
            } else {
              const successMessage = `[${result.response.response_message}]`
              CustomNotification.notif('warning', 'Warning', successMessage)
            }
            this.showCreteSimulasiForm = false
          })
          .catch(error => {
            ...
            CustomNotification.notif(notifError.status, notifError.status, successMessage)
            this.showCreteSimulasiForm = false
            if (error.response.status && error.response.status === 401) {
              localStorage.removeItem('accessToken')
              localStorage.removeItem('userData')
