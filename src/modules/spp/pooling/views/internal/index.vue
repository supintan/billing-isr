  ...
  <div>
    <b-row class="mb-4">
      <b-col class="text-right">
        <b-button variant="light" :to="{ name: 'add-pooling' }" class="mr-1 text-primary">
          ...
          >
            ...
            <template #cell(action)="data">
              <b-button
                ...
                :to="{ name: 'detail-pooling', params: {idpooling: data.item.id, name: data.item.client_name}}"
              >
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'verification_employee_full_name', label: 'Petugas Verifikasi' },
        { key: 'review_date_time', label: 'Tanggal Review' },
        { key: 'reviewer_employee_full_name', label: 'Petugas Review' },
        { key: 'pooling_status', label: 'Status Review' },
        { key: 'number_of_pooling_application', label: 'Jumlah Invoice' },
  ...
  methods: {
    ...
    getDataTable() {
      ...
      axios({
        url: URLServices.hkppooling.index,
        params: param,
        method: 'get',
        headers: {
          Authorization: `${TokenType} ${getToken()}`,
        ...
        .catch(() => {
          ...
          const succesMessage = `Gagal memperbaharui data. Mohon coba kembali.`
          CustomNotification.notif('danger', 'Warning', succesMessage)
        })
    },
