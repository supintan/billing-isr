  ...
  <div>
    ...
    <b-row class="mb-4">
      <b-col class="text-right">
        <b-button
          variant="light"
          :to="{name: 'store-invoice-manual-payment'}"
          v-if="$can('store', 'isr_manual_payment_submission')"
        ...
        <b-card no-body class="p-0 mb-4">
          ...
          >
            ...
            <template #cell(invoice_due_date)="data">
              {{ convertDate(data.item.invoice_due_date) }}
            </template>
            <template #cell(id)="data">
              <b-button
                ...
                :to="{
                  ...
                  params: { selectedPaymentID: data.item.id }, }"
                btn-sm
              >
...
export default {
  ...
  data() {
    return {
      searchTitle: '',
      isBusy: false, // ubah saat pakai WS
      seen: false,
      perPage: 10,
      ...
      fields: [
        { key: 'spectra_client_id', label: 'No. Pelanggan' },
  ...
  methods: {
    ...
    getDataTable() {
      ...
      axios({
        url: URLServices.manual_payment_submission.index,
        params: param,
        method: 'get',
        headers: {
