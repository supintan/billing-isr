  ...
  <b-overlay rounded="sm">
    ...
    <b-card class="shadow">
      <b-form ref="form">
        ...
        <b-row class="mb-1">
          <b-col lg="8" sm="8">
            <b-form-group label-cols="4" label="Denda" label-for="h-fine_amount">
              <b-input-group prepend="Rp.">
                <b-form-input
                  id="h-fine_amount"
                  v-model="FineAmount"
    ...
    <b-card class="mt-4 shadow">
        ...
        <b-row class="mb-1">
          <b-col lg="8" sm="8">
            <b-form-group label-cols="4" label="Catatan Keputusan" label-for="submission_notes">
              <b-form-textarea
                id="submission_notes"
  ...
  data() {
    return {
      ...
      fileOptions: [
        ...
        { item: 'Tidak Setuju', name: 'Tidak Setuju' },
      ],
      // DOWNLOAD PDF
      fileLoc: null,
      fileBase64: '',
          ...
          await axios({
            ...
            .then(result => {
              if (result.data.response_code === '00') {
                CustomNotification.notif('success', 'Berhasil memperbaharui', 'Halaman akan diarahkan ke index')
                this.$router.push({ name: 'review-invoice-manual-payment' })
              } else {
                CustomNotification.notif('warning', 'Gagal memperbaharui', result.data.response_message)
              }
              this.showCreteUserForm = false
            })
            .catch(error => {
              CustomNotification.notif('danger', 'Gagal memperbaharui', 'Mohon coba kembali')
              this.showCreteUserForm = false
