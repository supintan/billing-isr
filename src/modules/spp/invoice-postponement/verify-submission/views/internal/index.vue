          ...
          >
            ...
            <template #cell(id)="data">
              <b-button
                  ...
                  params: {
                    idAppeal: data.item.postponement_appeal_submissions_id,
                    postponementId : data.item.id,
                    appealId: data.item.postponement_appeal_submissions_id,
                    submissionId: data.item.postponement_submissions_id,
                    isReview: data.item.review_date_time != null,
                  },}"
                  btn-sm
              >
                Detail
              ...
              <!-- <b-button
                  variant="info"
                  @click="onClick(data.item.postponement_id)"
                  btn-sm
              >
                Download
              </b-button> -->
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        { key: 'no', label: 'No.' },
        { key: 'appeal_letter_number', label: 'No. Surat Permohonan', sortable: true },
        { key: 'upload_date_time', label: 'Tgl. Upload', sortable: true },
        { key: 'verification_date_time', label: 'Tgl. Verifikasi', sortable: true },
        { key: 'verification_employee_name', label: 'Petugas Verifikasi', sortable: true },
        { key: 'review_date_time', label: 'Tgl. Review', sortable: true },
        { key: 'reviewer_employee_name', label: 'Petugas Review', sortable: true },
        { key: 'review_status', label: 'Status Review', sortable: true },
        { key: 'invoice_id', label: 'No. Invoice' },
        ...
        { key: 'appeal_due_date', label: 'Tgl. Jatuh Tempo Baru', sortable: true },
        { key: 'id', label: 'Aksi' },
      ],
      items: [],
      showTable: false,
  ...
  methods: {
    ...
    convStatus(status) {
      if (status !== null) {
        return status ? 'DISETUJUI' : 'DITOLAK'
      }
      return '-'
    },
    ...
    getRequestParams(filter, currentPage, perPage) {
      ...
      if (this.sortBy) {
        ...
        params.sort_asc_desc = this.sortDesc ? 'Desc' : 'Asc'
      }
