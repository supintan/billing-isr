          ...
          >
            ...
            <template #cell(field6)="dataField">
              ...
              <template #cell(deposit_usage_amount)="data">
                {{ data.item.deposit_usage_amount.replaceAll('.00000', '') }}
              </template>
          </b-table>
        </b-col>
      </b-row>
    ...
    <!-- <b-card>
      <h5 class="text-primary">REVIEW PERMOHONAN PENGUNAAN DEPOSIT</h5>
        ...
        <b-col lg="12" sm="10">
          ...
          <b-col lg="12" sm="10">
          <b-form-group label-cols="3" label="File Surat Permohonan*" label-for="file">
            <b-form-file
              v-model="fileLoc"
              accept=".pdf"
              required
          ...
          <b-col lg="12" sm="12">
          <b-form-group label-cols="3" label="Catatan Keputusan" label-for="approveSubmissionNotesRequest">
            <b-form-textarea
...
export default {
  ...
  data() {
    return {
      ...
      // Client
      detailDepositApproveId: this.$route.params.detailDepositApproveId,
  ...
  methods: {
    ...
    onClick() {
      axios({
        ...
        params: {
          request_id: uuid.v4(),
          request_date_time: moment(new Date()).format('YYYY-MM-DD H:mm:s'),
          id: this.detailDepositApproveId,
        },
        headers: {
          Authorization: `${TokenType} ${getToken()}`,
        },
    ...
    getRequestParams() {
      ...
      params.deposit_usage_id = this.detailDepositApproveId
    ...
    async getDetailDeposit() {
      await axios({
        ...
        .then(response => {
          ...
          this.total_amount = this.detailDeposit[0].invoice_total_amount.replaceAll('.00000', '')
    ...
    async save() {
      ...
      if (this.selectedFormat === 'Setuju') {
        // if (1 !== 2) {
          await axios({
            url: URLServices.depositRequest.approve,
            method: 'post',
            data: {
              request_id: uuid.v4(),
              request_date_time: moment(new Date()).format('YYYY-MM-DD H:mm:s'),
      ...
      if (this.selectedFormat === 'Tidak Setuju') {
        // if (1 !== 2) {
          await axios({
            url: URLServices.depositRequest.reject,
            method: 'post',
            data: {
              request_id: uuid.v4(),
              request_date_time: moment(new Date()).format('YYYY-MM-DD H:mm:s'),
